...
parser.add_argument('--data_set_name', default='Tisza', help='dataset name')
...
def train_model():
    train_X, train_Y, train_label, test_X, test_Y = get_data()
    si_model = SiameseInception()
    our_model = si_model.get_model(input_size=[None, None, 3])
    ...
    for _epoch in range(MAX_EPOCH):
        ...
        for i in range(iter_on_epoch):
            ...
            temp_loss, temp_train_acc, temp_train_sen, temp_train_spe, temp_train_F1 = \
                our_model.train_on_batch(x=[train_1, train_2], y=label)
        ...
        binary_change_map = our_model.predict([test_X, test_Y])
        ...
        if save_best:
            cv.imwrite(os.path.join(RESULT_SAVE_PATH, os.path.join(DATA_SET_NAME, str(_epoch) + '_bcm.bmp')),
                       binary_change_map)
            # our_model.save(os.path.join(MODEL_SAVE_PATH, os.path.join(DATA_SET_NAME, str(_epoch) + '_model.h5')))
            save_best = False
...
def get_data():
    ...
    test_X, test_Y = load_test_data(path=path)
    for i in range(len(test_X)):
        test_X[i] = norm_img(test_X[i])
        test_Y[i] = norm_img(test_Y[i])
        # test_label[i] = test_label[i] / 255.
    # test_X = np.array(test_X) / 255.
    # test_Y = np.array(test_Y) / 255.
    # test_label = np.array(test_label) / 255.
    # test_label = np.reshape(, (test_label.shape[0], test_label.shape[1], test_label.shape[2]))
    for i in range(len(train_X)):
        train_X[i] = norm_img(train_X[i])
        train_Y[i] = norm_img(train_Y[i])
        train_label[i] = train_label[i] / 255.

    return train_X, train_Y, train_label, test_X, test_Y
