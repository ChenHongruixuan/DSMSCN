...
parser.add_argument('--learning_rate', type=float, default=1e-3, help='initial learning rate[default: 3e-4]')
parser.add_argument('--save_path', default='model_param', help='model param path')
parser.add_argument('--data_path', default='data/GF_2_1/train_data', help='dataset path')
...
class ChangeTrainer(object):
    ...
    def load_data(self):
        ...
        img_X = self.norm_img(img_X)  # (C, H, W)
        img_Y = self.norm_img(img_Y)
    ...
    def training(self):
        ...
        self.net, self.pred = self.sia_inc.get_model(Input_X=self.Input_X, Input_Y=self.Input_Y,
                                                     data_format='NHWC', is_training=self.is_training)  # (B, 2)
        self.loss = self._get_loss(label=self.label, logits=self.net)
        self.opt = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE)

        self.train_op = self.opt.minimize(loss=self.loss)
        best_loss = 100000
        iter_in_epoch = np.int(np.ceil(train_sz / BATCH_SZ))
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epoch_sz):
                ...
                for _iter in range(iter_in_epoch):
                    ...
                    batch_train_X = train_X[start_idx:end_idx]
                    batch_train_Y = train_Y[start_idx:end_idx]
                    batch_label = train_label[start_idx:end_idx]
                    loss, _, logits, pred, step = sess.run(
                        [self.loss, self.train_op, self.net, self.pred, self.global_step],
                        feed_dict={
                ...
                print(f"epoch {epoch + 1} , loss is {ave_loss}")
    ...
    def _get_loss(self, label, logits):
        loss = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(labels=label, logits=logits, name='weight_loss'))
        return loss

    def _load_train_data(self, path):
        with open(os.path.join(path, '9x9_ori_train_sample_X.pickle'), 'rb') as file:
            train_X = pickle.load(file)
        with open(os.path.join(path, '9x9_ori_train_sample_Y.pickle'), 'rb') as file:
            train_Y = pickle.load(file)
        with open(os.path.join(path, '9x9_ori_train_label.pickle'), 'rb') as file:
